...
echo '<h2>' . $tatarstan_news[heading] . '</h2><br>';
echo "<img width=300 height=200 src='" . $tatarstan_news[image] . "'/><br>";
echo '<h2 style=text-align:right;>' . $tatarstan_news[id] . '</h2><br>';
if ($added != "added") { 
    echo $add; // добавить в избранное
} else {
    echo $delete; // удалить из избранного
}
echo '<h4>' . $tatarstan_news[description] . '</h4><br>';     
$link = $tatarstan_news[image];
$link1 = str_replace($del1, $replace, $link);
$link2 = str_replace($del2, $replace, $link1);
echo '<a href="' . $link2 . '.php">Подробнее о новости...</a><hr>';

    foreach ($select_tat1 as $key => $value):
